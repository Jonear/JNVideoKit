...
//    assetWriter = [[AVAssetWriter alloc] initWithURL:movieURL fileType:AVFileTypeQuickTimeMovie error:&error];
    ...
    if (outputSettings ==  nil) {
        NSMutableDictionary * t_outputSettings = [[NSMutableDictionary alloc] init];
        [t_outputSettings setObject: AVVideoCodecH264 forKey: AVVideoCodecKey];
        [t_outputSettings setObject: [NSNumber numberWithInt: videoSize.width] forKey: AVVideoWidthKey];
        [t_outputSettings setObject: [NSNumber numberWithInt: videoSize.height] forKey: AVVideoHeightKey];
        
        outputSettings = t_outputSettings;
    }
...
- (void)startRecording:(BOOL)isDelay
{
    ...
    if (isDelay) {
        [assetWriter startSessionAtSourceTime:CMTimeMake(4, 120)]; //为了去除多段视频前面的小黑屏
    } else {
        [assetWriter startSessionAtSourceTime:CMTimeMake(60, 120)];
    }
    ...
    if(assetWriter.status == AVAssetWriterStatusWriting) {
        ...
        if ([assetWriter respondsToSelector:@selector(finishWritingWithCompletionHandler:)]) {
            ...
            [assetWriter finishWritingWithCompletionHandler:^{
                NSLog(@"~~~~finish write video!");
//                postNotifacation(FreeVideoRecordWriteVideoFinsh, nil, nil);
            }];
        }
        else {
            // Not running iOS 6
...
//        postNotifacation(FreeVideoRecordWriteVideoFailed, nil, nil);
    }
}
