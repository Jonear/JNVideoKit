...
//    UIImageWriteToSavedPhotosAlbum([[self.filter imageFromCurrentlyProcessedOutput] rotatedByDegrees:0], self, @selector(image:didFinishSavingWithError:contextInfo:), nil);
}
...
- (void)takePhoto {
    ...
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        @autoreleasepool {
            ...
            image = [image rotatedByDegrees:90.0f];
            image = [image croppedImage:CGRectMake(0, 0, 640, 640)];
...
- (id)initWithSessionPreset:(NSString *)sessionPreset cameraPosition:(AVCaptureDevicePosition)cameraPosition highVideoQuality:(BOOL)isHighQuality {
    ...
    gpuImageView = [[GPUImageView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_WIDTH)];
...
- (void)setRotation:(GPUImageRotationMode)newRotationMode {
    [rotationFilter setRotation:newRotationMode];
}
